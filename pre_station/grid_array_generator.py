...
# load scalar_inflow_and_outflow_array and scalar_volume_array
...
grid_inflow_and_outflow_array = np.zeros((time_slot_num, filter_num, 2, grid_side_length, grid_side_length), dtype = 'int32')
# grid_volume
grid_volume_array = np.zeros( (time_slot_num, filter_num, 2, grid_side_length, grid_side_length), dtype = 'int32' )
...
for time_slot_index in range(time_slot_num):
    ...
    for current_station_index in range(filter_num):
        ...
        for cluster_index in cluster_list:
            ...
            # inflow
            grid_inflow_and_outflow_array[time_slot_index, current_station_index, 0, c_x, c_y] += scalar_inflow_and_outflow_array[time_slot_index, cluster_index, 0]

            # outflow
            grid_inflow_and_outflow_array[time_slot_index, current_station_index, 1, c_x, c_y] += scalar_inflow_and_outflow_array[time_slot_index, cluster_index, 1]

            # volume
            grid_volume_array[time_slot_index, current_station_index, 0, c_x, c_y] += scalar_volume_array[time_slot_index, cluster_index, 0]
            grid_volume_array[time_slot_index, current_station_index, 1, c_x, c_y] += scalar_volume_array[time_slot_index, cluster_index, 1]

print('grid_inflow_and_outflow_array:')
print(grid_inflow_and_outflow_array)
print('grid_inflow_and_outflow_array.shape:')
print(grid_inflow_and_outflow_array.shape)

print('grid_volume_array:')
print(grid_volume_array)
print('grid_volume_array.shape:')
print(grid_volume_array.shape)

np.save('npy_files/grid_inflow_and_outflow_array.npy', grid_inflow_and_outflow_array)
np.save('npy_files/grid_volume_array.npy', grid_volume_array)
