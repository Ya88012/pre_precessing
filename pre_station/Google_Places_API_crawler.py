...
for s in range(len(df)):
    ...
    url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=" + str(df.iloc[s, 3]) + "," + str(df.iloc[s, 4]) + "&radius=" + str(radius) + "&rankBy=google.maps.places.RankBy.DISTANCE&key=AIzaSyAfCmoIAchzhCyRnry9Lc_Ya5zTcRDFuo4&language=zh-TW"
    print(url)
    ...
    if response and response.get("status") == "REQUEST_DENIED":
        ...
        while(True):
            ...
            if "next_page_token" in response:
                ...
                url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=" + str(df.iloc[s, 3]) + "," + str(df.iloc[s, 4]) + "&pagetoken=" + next_token + "&key=AIzaSyAfCmoIAchzhCyRnry9Lc_Ya5zTcRDFuo4"
                time.sleep(5)
                response = requests.request("GET", url, headers = headers, data = payload).json()
                if response and response.get("status") == "REQUEST_DENIED":
                    print(response)
